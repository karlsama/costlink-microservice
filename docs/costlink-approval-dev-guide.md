...
**重要**：`approval.completed` 的 payload 必须包含以下字段。事件类 `ApprovalCompletedEvent` 在 `common/mq/event/` 中已定义，审批服务发布前需填充全部字段：

```java
// approval.completed 事件体（全部必填）
ApprovalCompletedEvent event = new ApprovalCompletedEvent();
event.setReimbursementId(instance.getReimbursementId());
event.setInstanceId(instance.getId());
event.setAction("APPROVED");                              // APPROVED / REJECTED
event.setTitle(instance.getTitle());                      // 报销事由（通知服务渲染用）
event.setAmount(instance.getTotalAmount());               // 金额（通知服务渲染用）
event.setApplicantId(instance.getApplicantId());          // 收件人
```
...
- [ ] 全部通过/驳回时发布 `approval.completed`，用 `common/mq/event/ApprovalCompletedEvent`，**六个字段全部填充**
